 ...
 * Retorna un objeto { total, ingredientes } donde ingredientes es un array de:
 * { idIngrediente, nombre, cantidad, unitCost, subtotalCost }
 */
const calcularCostoConDesgloseReceta = async (idTorta, userId) => {
  try {
    ...
    for (const receta of recetas) {
      ...
      if (!ingrediente) {
        ingredientesDetalles.push({
          idIngrediente: Number(ingredienteId),
      ...
      costoTotal += subtotal;
      ingredientesDetalles.push({
        idIngrediente: Number(ingrediente.id),
    ...
    return { total: costoTotal, ingredientes: ingredientesDetalles };
  } catch (error) {
